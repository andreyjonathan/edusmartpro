...
const type = ref(props.type || 'student');
...
watch([search, date, status, className, type], updateFilters);
...
const getSubtitle = () => {
  switch (props.type) {
    case 'teacher': return 'Monitor and manage daily presence of teaching staff.';
    case 'employee': return 'Track and record attendance for administrative and support staff.';
    default: return 'Monitor and manage daily student presence and participation.';
  }
};
    ...
    <div class="space-y-8">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-slate-500 mt-1">{{ getSubtitle() }}</p>
        </div>
        <div class="flex items-center gap-3">
          ...
          <Link 
            :href="route('attendances.create', { type: props.type })"
      ...
      <Card class="p-6">
        <div class="flex flex-col lg:flex-row gap-4 items-end">
          <div class="flex-1 space-y-2 w-full">
            <label class="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">{{ props.type === 'student' ? t.attendance.search : t.attendance.searchName }}</label>
            <div class="relative group">
                ...
                v-model="search"
                type="text" 
                :placeholder="props.type === 'student' ? t.attendance.searchPlaceholder : t.attendance.searchNamePlaceholder" 
          ...
          <div class="w-full lg:w-48 space-y-2">
            <label class="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">{{ t.attendance.date }}</label>
            <input 
              v-model="date"
              type="date" 
          ...
          <div v-if="props.type === 'student'" class="w-full lg:w-48 space-y-2">
            <label class="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">{{ t.attendance.class }}</label>
            <select 
              v-model="className"
              class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-sm text-slate-900 focus:ring-4 focus:ring-indigo-50/50 focus:border-indigo-500 focus:bg-white transition-all outline-none"
            >
          ...
          <div class="w-full lg:w-48 space-y-2">
            <label class="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">{{ t.attendance.status }}</label>
            <select 
              v-model="status"
              class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-sm text-slate-900 focus:ring-4 focus:ring-indigo-50/50 focus:border-indigo-500 focus:bg-white transition-all outline-none"
            >
              <option value="">{{ t.attendance.allStatus }}</option>
              <option v-for="s in ['Present', 'Absent', 'Late', 'Sick']" :key="s" :value="s">{{ t.common[s.toLowerCase()] || s }}</option>
            </select>
          </div>
        </div>
      </Card>
      ...
      <Card class="overflow-hidden border-none shadow-xl shadow-slate-200/50">
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-slate-50/50 border-b border-slate-100">
                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">{{ props.type === 'student' ? t.common.student : t.attendance.status }}</th>
                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">{{ t.attendance.date }}</th>
                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">{{ t.attendance.status }}</th>
                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">{{ t.attendance.notes }}</th>
              ...
              <tr v-for="attendance in attendances.data" :key="attendance.id" class="group hover:bg-slate-50/50 transition-colors">
                <td class="px-6 py-4">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <div class="text-sm font-bold text-slate-900">{{ attendance.attendant.name }}</div>
                      <div v-if="props.type === 'student' && attendance.attendant.nis" class="text-[11px] font-medium text-slate-400">NIS: {{ attendance.attendant.nis }}</div>
                      <div v-if="props.type === 'teacher' && attendance.attendant.nip" class="text-[11px] font-medium text-slate-400">NIP: {{ attendance.attendant.nip }}</div>
                      <div v-if="props.type === 'employee' && attendance.attendant.nik" class="text-[11px] font-medium text-slate-400">NIK: {{ attendance.attendant.nik }}</div>
                    </div>
                  </div>
                </td>
        ...
        <div class="px-6 py-4 bg-slate-50/30 border-t border-slate-100 flex items-center justify-between">
          <p class="text-xs font-medium text-slate-500">
            {{ t.common.showing }} {{ attendances.from || 0 }} {{ t.common.to }} {{ attendances.to || 0 }} {{ t.common.of }} {{ attendances.total }} {{ t.common.records }}
          </p>
