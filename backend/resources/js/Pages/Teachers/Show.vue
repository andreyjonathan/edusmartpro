      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex flex-col gap-4">
          ...
          <div class="flex items-center gap-6">
            ...
            <div>
              <h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ teacher.name }}</h1>
              <div class="flex items-center gap-3 mt-2">
                ...
                <div 
                    :class="[
                        ...
                        teacher.status === 'Active' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-slate-100 text-slate-500 border-slate-200'
                    ]"
                  >
            ...
            <Card class="p-6 space-y-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="text-[11px] font-bold text-slate-400 uppercase tracking-tight">NIP</div>
                            <div class="text-sm font-bold text-slate-700">{{ teacher.nip }}</div>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 border border-slate-100">
                        ...
                        <div>
                            ...
                            <div class="text-sm font-bold text-slate-700">{{ formatDate(teacher.created_at) }}</div>
                        </div>
                    </div>
                </div>
            </Card>
                ...
                <div class="space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="overflow-hidden">
                            ...
                            <div class="text-sm font-bold text-slate-700 truncate">{{ teacher.email || '-' }}</div>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div class="text-sm font-bold text-slate-700">{{ teacher.phone || '-' }}</div>
                        </div>
                    </div>
                </div>
            </Card>
            ...
            <Card class="p-8 space-y-8">
                <div>
                    ...
                    <p class="text-slate-600 leading-relaxed bg-slate-50 p-6 rounded-2xl border border-slate-100 min-h-[120px]">
                        {{ teacher.address || 'No address information provided.' }}
                    </p>
                </div>

                <div class="pt-8 border-t border-slate-100">
                    <h3 class="text-lg font-bold text-slate-800 mb-6">Teaching Assignments</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="p-4 rounded-2xl border border-slate-100 bg-white shadow-sm flex items-center gap-4">
                            <div class="w-12 h-12 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold">
                                {{ teacher.subject.substring(0, 2).toUpperCase() }}
                            </div>
                            <div>
                                <div class="text-sm font-bold text-slate-900">{{ teacher.subject }}</div>
                                <div class="text-xs text-slate-400">Primary Subject</div>
                            </div>
                        </div>
                    </div>
                </div>
